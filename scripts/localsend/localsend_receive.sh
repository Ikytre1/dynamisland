...
[ ! -d "$SAVE_DIR" ] && SAVE_DIR="$HOME/Documents"
mkdir -p "$SAVE_DIR"
...
class LocalSendHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        # 3. Réception des données binaires
            ...
            bytes_read = 0
            last_pct_sent = -1
            fname_for_progress = os.path.basename(dest_path)
            sender_alias = session.get("alias", "Unknown Device")
            with open(dest_path, "wb") as f:
                while bytes_read < content_length:
                    ...
                    # Émission de la progression (throttlée à chaque palier de 1%)
                    if content_length > 0:
                        pct = int((bytes_read * 100) / content_length)
                    else:
                        pct = 100
                    if pct != last_pct_sent:
                        last_pct_sent = pct
                        print(f"PROGRESS\t{sender_alias}\t{fname_for_progress}\t{pct}", flush=True)

            if last_pct_sent != 100:
                print(f"PROGRESS\t{sender_alias}\t{fname_for_progress}\t100", flush=True)

            print(f"RECEIVED\t{sender_alias}\t{os.path.basename(dest_path)}", flush=True)
            os.system(f'notify-send "LocalSend" "Received : {os.path.basename(dest_path)}" -i emblem-ok-symbolic')
