 ...
 * depends on the system icon theme or external asset files[cite: 4]. Set `name` to pick a
 * glyph, `color` to tint it; stroked glyphs use `stroke` width, filled glyphs
 * (media transport) paint solid[cite: 4]. Paths live in a 24x24 space and scale to the
 * item's size[cite: 4]. Each glyph's actual bounding box is centred within the item on
 * both axes, so glyphs with differing path extents share one optical baseline[cite: 4].
 */
Item {
    id: root
    ...
    Shape {
        ...
        // a half pixel renders every 1.2px stroke as a soft smear[cite: 4].
